...
const AnnotationTool = () => {
  ...
  useEffect(() => {
    async function findFrame() {
      ...
      const frame = frames.find(
        (f: any) => f.name === trainingState.selectedFrame
      ); // fetch the selected frame
      if (frame) {
        const r = frame.roles.map((r: any) => {
          return {
            ...
            color: `hsl(${Math.floor(Math.random() * 360)}, ${Math.floor(
              Math.random() * 81 + 20
            )}%, ${Math.floor(Math.random() * 24 + 72)}%`, // random lighter color
          };
        });
        r.unshift({
          // first element in roles array would be the lexical units
          name: "Lexical Units",
          color: `hsl(${Math.floor(Math.random() * 360)}, ${Math.floor(
            Math.random() * 81 + 20
          )}%, ${Math.floor(Math.random() * 24 + 72)}%`, // random lighter color
        ...
        setWords(
          trainingState.inputText!.split(" ").map((text, index) => {
  ...
  const clickRole = (i: number) => {
    // index of role
    setActiveItemIdx(i);
  };

  const clickText = (word: Word) => {
    // selected word in input string
    if (activeItemIdx !== null) {
      let w = words.slice(); // copy of the words array

      if ("role" in word && word.role === roles[activeItemIdx]) {
        // this word has already been annotated by the same role
        delete word["role"]; // remove the highlight
      } else {
        // either no highlight or replace the highlight with a different color
  ...
  return (
    <div>
      <Flex align={"center"} gap={"xs"} mb={17}>
              ...
              >
                <Button
                  ...
                  style={{
                    height: "35px",
                    backgroundColor: `${
                      activeItemIdx === 0 && roles.length > 0
                        ? roles[0].color
                        : "white"
                    }`,
                  }}
                ...
                {roles.slice(1).map((role, index) => {
                  return (
                    <div>
                      <Button
                        onClick={() => clickRole(index + 1)}
                        style={{
                          height: "35px",
                          backgroundColor: `${
                            activeItemIdx !== null &&
                            roles[activeItemIdx].name === role.name
                              ? role.color
                              : "white"
                          }`,
                        }}
                    ...
                    <div style={{ textAlign: "center" }}>
                      {word.role ? (
                        <Text
                          ...
                          fw={500}
                          style={{
                            color: tinycolor(word.role.color)
                              .darken(25)
                              .toString(),
                          }}
                        >
                          {word.role.name === "Lexical Units"
                            ? "LU"
                            : word.role.name}
                        </Text>
                      ) : (
                      ...
                      <Button
                        onClick={() => clickText(word)}
                        key={index}
                        style={{
                          height: "23px",
                          backgroundColor: `${
                            word.role ? word.role.color : "white"
                          }`,
                          padding: "3px",
                          width: "100%",
                        }}
