  ...
  var Visualizer = function (dispatcher, svgId, webFontURLs) {
    ...
    var setData = function (_sourceData) {
      ...
      $.each(sourceData.attributes, function (attrNo, attr) {
        ...
        var attrType =
          eventAttributeTypes[attr[1]] || entityAttributeTypes[attr[1]];
        var attrValue =
          attrType && attrType.values.byName[attrType.bool || attr[3]];
      ...
      $.each(sourceData.comments, function (commentNo, comment) {
        ...
        if (comment[0] instanceof Array && comment[0][0] == "sent") {
          ...
          $.each(commentEntities, function (entityId, entity) {
            ...
            if (
              commentPriority(comment[1]) > commentPriority(entity.shadowClass)
            ) {
      ...
      $.each(data.chunks, function (chunkNo, chunk) {
        ...
        $.each(chunk.fragments, function (fragmentNo, fragment) {
          ...
          $.each(fragment.span.attributes, function (attrType, valType) {
            ...
            // instead of using the first attribute def found
            var attr =
              eventAttributeTypes[attrType] || entityAttributeTypes[attrType];
            if (!attr) {
              // non-existent type
              warning = true;
              return;
      ...
      var textSizes = getTextMeasurements(
        ...
        function (fragment, text) {
          if (fragment instanceof Fragment) {
            ...
            firstChar -=
              textUpToFirstChar.length - textUpToFirstCharUnspaced.length;
            lastChar -=
              textUpToLastChar.length - textUpToLastCharUnspaced.length;
            ...
            var pos =
              fragment[2] - (textUpToChar.length - textUpToCharUnspaced.length);
            if (pos < 0) pos = 0;
            if (!pos) {
              // start
    ...
    var renderDataReal = function (sourceData) {
      ...
      if (
        (sourceData &&
          ...
          (sourceData.document !== doc || sourceData.collection !== coll)) ||
        drawing
      ) {
        redraw = true;
        dispatcher.post("doneRendering", [coll, doc, args]);
      ...
      $.each(data.chunks, function (chunkNo, chunk) {
        ...
        if (chunk.lastSpace) {
          ...
          if (spacePos || !chunkNo || !chunk.sentence) {
            ...
            for (var i = spacePos; i < spaceLen; i++)
              spaceWidth += spaceWidths[chunk.lastSpace.charAt(i)] || 0;
        ...
        $.each(chunk.fragments, function (fragmentNo, fragment) {
          ...
          if (shadowRect) {
            $(shadowRect).attr(
              "y",
              yy - rectShadowSize - Configuration.visual.margin.y - span.floor
            );
          }
          if (markedRect) {
            $(markedRect).attr(
              "y",
              yy - markedSpanSize - Configuration.visual.margin.y - span.floor
            );
          }
          ...
          if (fragment.drawCurly) {
            ...
            var bottom =
              yy + hh + Configuration.visual.margin.y - span.floor + 1;
            svg.path(
              fragment.group,
              svg
                .createPath()
            ...
            $.each(span.incoming, function (arcId, arc) {
              ...
              if (origin.row) {
                ...
                var smallestLabelWidth =
                  sizes.arcs.widths[labels[labelNo]] + 2 * minArcSlant;
        ...
        if (chunk.sentence || chunkDoesNotFit) {
          ...
          if (hasLeftArcs) {
            var adjustedCurTextWidth =
              sizes.texts.widths[chunk.text] + arcHorizontalSpacing;
      ...
      $.each(data.arcs, function (arcNo, arc) {
        ...
        var leftToRight =
          originSpan.headFragment.towerId < targetSpan.headFragment.towerId;
        var left, right;
        if (leftToRight) {
        ...
        var arcDesc =
          relationTypesHash[arc.type] || relationTypesHash[noNumArcType];
        ...
        var symmetric =
          arcDesc && arcDesc.properties && arcDesc.properties.symmetric;
        ...
        for (var rowIndex = leftRow; rowIndex <= rightRow; rowIndex++) {
          ...
          if (row.chunks.length) {
            ...
            if (collapseArcSpace) {
              var fromIndex2R =
                rowIndex == leftRow ? fromIndex2 : row.heightsStart;
            ...
            if (rightBox && rowIndex == rightRow) {
              ...
              to = rtlmode
                ? 0
                : canvasWidth - 2 * Configuration.visual.margin.y;
            }

            if (collapseArcs) {
              var arcCacheKey =
                arc.type + " " + rowIndex + " " + from + " " + to;
              if (rowIndex == leftRow)
            ...
            var myArrowHead =
              (styleMap[arcRef] && styleMap[arcRef]["arrowHead"]) ||
              (arcDesc && arcDesc.arrowHead) ||
              (spanTypes.ARC_DEFAULT && spanTypes.ARC_DEFAULT.arrowHead);
            var arrowName =
              (symmetric
            ...
            var myLabelArrowHead =
              (styleMap[arcRef] && styleMap[arcRef]["labelArrow"]) ||
              (arcDesc && arcDesc.labelArrow) ||
              (spanTypes.ARC_DEFAULT && spanTypes.ARC_DEFAULT.labelArrow);
            if (myLabelArrowHead) {
              var labelArrowName =
              ...
              var labelArrowDecl =
                labelArrowType && "url(#" + labelArrowType + ")";
            ...
            if (leftBox && rowIndex == leftRow) {
              var cornerx =
                from + (rtlmode ? -ufoCatcherMod : ufoCatcherMod) * arcSlant;
            ...
            if (!symmetric) {
              myArrowHead =
                ...
                (spanTypes.ARC_DEFAULT && spanTypes.ARC_DEFAULT.arrowHead);
              arrowName =
                (leftToRight ? myArrowHead || "triangle,5" : "none") +
                "," +
                color;
            }
            var arrowType = arrows[arrowName];
      ...
      if (rtlmode) {
        ...
        svg.path(
          sentNumGroup,
          svg.createPath().move(sentNumMargin, 0).line(sentNumMargin, y)
        );
      }

      // resize the SVG
      var width =
        maxTextWidth + sentNumMargin + 2 * Configuration.visual.margin.x + 1;
      ...
      if ((id = target.attr("data-span-id"))) {
        ...
        if (that.arcDragOrigin) {
          ...
          var addArcAndSpan = function (arc, span) {
            if (arc.equiv) {
              equivs[
                arc.eventDescId.substr(0, arc.eventDescId.indexOf("*", 2) + 1)
              ] = true;
              var eventDesc = data.eventDescs[arc.eventDescId];
              $.each(
                eventDesc.leftSpans.concat(eventDesc.rightSpans),
