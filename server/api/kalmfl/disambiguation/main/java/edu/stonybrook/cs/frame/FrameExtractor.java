...
public class FrameExtractor {
	...
	private static final String sentenceEmbeddingGlosses = "resources/embeddings/filler_gloss_framenet.txt";
	...
	public static ArrayList<Frame> getFrameExtractionResult(String sourceFile) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(candidateParseDir + sourceFile))) {
		    ...
		    int sentenceIndex = 0;
			FileOutputStream fileOutputStream = new FileOutputStream(sentenceEmbeddingGlosses);
		    ...
		    while ((line = br.readLine()) != null) {  // first you meet frame name, construct a frame; then you meet role fillers, fill the roles
		    	if (line.contains("==")) {
					...
					List<Thread> threads = role.createSematicScoreComputationThreads(outputStreamWriter); // every role and filler has a thread list
			...
			System.out.println("Node query elapsed time: " + elapsedSeconds + " seconds");
			startTime = System.nanoTime();
		...
		elapsedTime += endTime - startTime;
		double elapsedSeconds = elapsedTime / 1000000000.0;
		System.out.println("Path search elapsed time: " + elapsedSeconds + " seconds");
		return frameList;
	}
