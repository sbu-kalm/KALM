...
public class Role {
	 ...
	 * @throws IOException
	 */
	public List<Thread> createSematicScoreComputationThreads(OutputStreamWriter writer) throws IOException {
		...
		if (roleName.equals("Time")) {
			...
			glossLine = filler + " :: null :: a time value\n";
			writer.write(glossLine);
			return null;
		}
		...
		if (roleDataType != null && roleDataType.equals("Integer")) {
			...
			glossLine = filler + " :: null :: an integer\n";
			writer.write(glossLine);
			return null;
		}
		...
		if (roleDataType != null && roleDataType.equals("Currency")) {
			...
			glossLine = filler + " :: null :: some currency\n";
			writer.write(glossLine);
			return null;
		}
		...
		for (BabelSynset synset : fillerSynsets) {  // iterate every synset of the role-filler, rewrite glosses
			...
			glossLine = filler + " :: " + synsetId + " :: " + mainGloss + "\n";
			writer.write(glossLine);
		}
		...
		if (entityPronouns.contains(filler)) {
			...
			glossLine = filler + " :: " + fillerScoreTriplet.fillerSynsetID + " :: " + fillerScoreTriplet.fillerGloss + "\n";
			writer.write(glossLine);
		}
...
//			fillerScoreTriplets.add(fillerScoreTriplet);
		...
		if (fillerScoreTriplets.size() == 0) {
			if (!hasExactMatch) {
				...
				glossLine = filler + " :: null :: not found\n";
				writer.write(glossLine);
			}
			return null;
		}
