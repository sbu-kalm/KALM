...
class Processor:
    ...
    def stanza_parse(self):
        ...
        for sent_id, raw_sent in tqdm(enumerate(self.text, 1), total=len(self.text)):
    ...
    def get_coref_sentences(self):
        ...
        for sent_id, sentence in enumerate(self.sentences, 1):
            if sentence.has_coref():
                try:
                    coref_rslvd = self.nlp_coref(self.raw_sentences[sent_id - 1])
                    if coref_rslvd._.has_coref:
                        coref = str(coref_rslvd._.coref_clusters[0]).split(':')[0]
                        coref_sentences[sent_id] = (self.raw_sentences[sent_id-1], coref)
                    else:
    ...
    # def get_clause_level_rejected_sentences(self):

    #     for sent_id, sentence in enumerate(self.sentences, 1):

    #         if sent_id not in self.rejected_sentences:
    #             if sentence.can_be_split():
    #                 sentence.split_into_clauses()
    #                 if not sentence.is_clause_level_verb_centric():
    #                     self.rejected_sentences[sent_id] = self.raw_sentences[sent_id-1]
        ...
        for sent_id, sentence in enumerate(self.sentences, 1):
            ...
            for parse_id, graph in enumerate(sentence.graphs, 1):
                ...
                for word in graph:
                    ...
                    edges = ','.join(edges)
                    lemma = word[3].replace('-', '_').replace('\'s', 's') \
                            if word[9] == 'O' and word[4] != 'PROPN' or 'MONEY' in word[9] or 'DATE' in word[9] \
                            else \
                            word[2].replace('-', '_').replace('\'s', 's')

                    f.write("word(index(%s),%s,[%s],edge(index(%s),%s),%s,%s,index(%s),%s,%s,%s).\n" %
                            (str(word[0]) + ',' + str(word[1]) + ',' + str(parse_id), 
                             '\'' + lemma + '\'' if not lemma.isdigit() else lemma,
                             edges,
                             word[7][0],
                             word[7][1].replace(':', '_'),
                             word[4].lower(),
